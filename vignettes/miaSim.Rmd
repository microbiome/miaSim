...
miaViz::plotSeries(glvmodel, x = "time")
...
miaViz::plotSeries(hubbellmodelRates, x = "time")
```
...
miaViz::plotSeries(crmodel, x = "time")
...
n_simulations <- lapply(seq_len(2), function (i) {do.call(simulateConsumerResource, params)})

# Visualize the model for the first instance
miaViz::plotSeries(n_simulations[[1]], "time")
...
communities <-  t(sapply(n_simulations, function (x) {assay(x, "counts")[, which.max(x$time)]}))
...
n_simulations_2 <- .generateSimulations(
    model = "simulateConsumerResource",
    params_list = params, param_iter = params_iter, n_instances = 1, t_end = 20
)

estimatedA <- .estimateAFromSimulations(n_simulations, n_simulations_2, n_instances = 1,
