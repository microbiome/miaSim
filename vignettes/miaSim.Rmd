...
rickermodel <- simulateRicker(n_species=4, A = A_normal, tend=100, norm = FALSE)
```
...
The abundance matrices can be converted into `SummarizedExperiment` object with `convertToSE`. This provides access to a broad range of tools for microbiome analysis that support this format (see [microbiome.github.io](http://microbiome.github.io)). More examples on `SummarizedExperiment` object manipulation and analysis can be found at [OMA Online Manual](https://microbiome.github.io/OMA).

This example shows how to generate taxonomic abundance data, including sample metadata, and convert this into into `SummarizedExperiment`.
...
SE_GLV <- convertToSE(assay = t(glvmodel$matrix[,1:4]),
                      colData = df)
```
...
HubbellSE <- convertToSE(assay = t(hubbellmodelRates$matrix[,1:3]),
                        colData = hubbellmodelRates$matrix[,"time"],
...
It is also possible to incorporate phylogenetic tree information and convert into `TreeSummarizedExperiment`. For further details, see:
