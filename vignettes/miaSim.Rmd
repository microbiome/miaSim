...
SE_GLV <- TreeSummarizedExperiment(
                      assays = list(counts = glvmodel), 
                      colData = df)
```
...
tse <- TreeSummarizedExperiment(
                        assays = list(counts = hubbellmodelRates$counts),  
...
data in the `TreeSummarizedExperiment` format (see e.g. microbiome.github.io).

For instance, to plot population density we can use the `miaViz` package:

```{r eval=FALSE, include=FALSE}
library(miaViz)
HubbellDensityPlot <- plotAbundanceDensity(tse, abund_values = "counts")
```
