...
#' is implemented here as proposed by Fisher and Mehta in PLoS ONE 2014. The function returns a
...
#' @param sigma noise level, if set to a non-positive value, no noise is added (default: \code{sigma = 0.05})
#' @param explosion.bound boundary for explosion (default: \code{explosion.bound = 10^8})
...
#' class object containing matrix with species abundance as rows and time points as columns
...
setGeneric("simulateRicker", signature = "n.species",
           function(n.species, A, x = runif(n.species),
                    K = runif(n.species), sigma=0.05, explosion.bound=10^8, tskip = 0, tend, norm = FALSE)
          ...
          function(n.species, A, x = runif(n.species),
            ...
            for(t in seq(from=2,to=tend)){
              ...
              if(max(x) > explosion.bound){
                ...
                stop(paste("Explosion for species", which(x==max(x))))
              }
              if(length(x[x<0]) > 0){
